...
+ (id)downloaderWithURL:(NSURL *)url delegate:(id<SDWebImageDownloaderDelegate>)delegate imageName:(NSString *)imageName
{
    return [self downloaderWithURL:url delegate:delegate userInfo:nil imageName:imageName];
}

+ (id)downloaderWithURL:(NSURL *)url delegate:(id<SDWebImageDownloaderDelegate>)delegate userInfo:(id)userInfo imageName:(NSString *)imageName
{
    return [self downloaderWithURL:url delegate:delegate userInfo:userInfo lowPriority:NO imageName:imageName];
}

+ (id)downloaderWithURL:(NSURL *)url delegate:(id<SDWebImageDownloaderDelegate>)delegate userInfo:(id)userInfo lowPriority:(BOOL)lowPriority imageName:(NSString *)imageName
{
...
#pragma clang diagnostic pop
    ...
    downloader.imageName = imageName;
...
- (void)connection:(NSURLConnection *)aConnection didReceiveData:(NSData *)data
{
    ...
    if (self.progressive && expectedSize > 0 && [delegate respondsToSelector:@selector(imageDownloader:didUpdatePartialImage:)])
    {
        ...
        CGImageSourceRef imageSource = CGImageSourceCreateIncremental((CFDictionaryRef)NULL);
...
- (void)connectionDidFinishLoading:(NSURLConnection *)aConnection
{
    if ([delegate respondsToSelector:@selector(imageDownloader:didFinishWithImage:)])
    {
        ...
        if(image){
            ...
            enum ImageType imageType = [[self.userInfo objectForKey:kImageType] intValue];
            switch (imageType)
            {
                ...
                {
                    image = [image imageScaledInterceptToSize:CGSizeMake(200, 200)];
                }
                    break;
//                case ImageType_ImageMessage://200
//                {
//                    image = [image imageScaledInterceptToSize:CGSizeMake(200, 200)];
//                }
                    break;
