...
-(void)chageNoteLastTimeForNewUpdateTime:(long long)updateTime{ //当发送新的Note和Comment时更新
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:kNoteOrCommentUpdateTimeChanged object:@(NO)];
}
...
-(BOOL)chageNoteLastTime:(long long)lastTime andCurTime:(long long)curTime{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:kNoteOrCommentUpdateTimeChanged object:@(bShowTip)];
    return YES;
}
...
-(void)_showLoginVCAndCancelNetCall:(BOOL)animated{
    ...
    [[ACConfigs getRootViewController] ACpresentViewController:[[ACLoginViewController2 alloc] init] animated:YES completion:nil];
}
...
-(void)newAppVersionCheckWithBlock:(void (^)(ACConfigs* pConfig,int newAppVersionCheck_Result_Type)) pFunc{
    ...
    if(NULL==pURLEnd){
        ...
        pFunc(self,newAppVersionCheck_Result_Type_No_Update);
        return;
    }
                   ...
                   if(!bIsFail){
                       ...
                       if(ResponseCodeType_Nomal==[[responseDic objectForKey:kCode] intValue]&&
                          ((NSString*)[responseDic objectForKey:@"download"]).length>5){
                           ...
                           self.appNewVersionInfo = responseDic;
                           if(bNeedPostkHaveNewAppVerion){
                               [[NSNotificationCenter defaultCenter] postNotificationName:kHaveNewAppVerion object:nil];
                           }
                       }
                   }
                   pFunc(wself,newAppVersionCheck_Result_Type);
                   pFunc(self,newAppVersionCheck_Result_Type);
               }];
}
